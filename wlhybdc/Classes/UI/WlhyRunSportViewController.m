 //
//  WlhyRunSportViewController.m
//  wlhybdc
//
//  Created by linglong meng on 12-10-24.
//  Copyright (c) 2012年 linglong meng. All rights reserved.
//

#import "WlhyRunSportViewController.h"
...
@interface WlhyRunSportViewController () <IFlySpeechSynthesizerDelegate, UITableViewDelegate, UITableViewDataSource, UINavigationBarDelegate, UIAlertViewDelegate, UIActionSheetDelegate, WlhyXMPPDelegate, DropButtonViewDelegate>
{
    float totalDistance;
    float totalTime;
...
@property(strong, nonatomic) IBOutlet UILabel *currentSlopeLabel;
...
@implementation WlhyRunSportViewController
...
- (void)viewDidLoad
{
    ...
    for (int i = 1; i < 10; i++) {
        [_animationImageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"run0%i.png", i]]];
    }
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    */
    ...
    _currentSlopeLabel.text = WlhyString([NSString stringWithFormat:@"%@°", [stepDic objectForKey:@"slope"]]);
...
- (void)getSportsData
{
    ...
    NSLog(@"%@ , %i", _barDecode, [_commonPrescTag intValue]);
    ...
    if ([_commonPrescTag intValue] < 0) {
        ...
        _dataSource = prescDic;
        _prescriptionId = [NSString stringWithFormat:@"PBJ_%i", _commonPrescTag.intValue];
    }
...
- (void)runTimerHandler:(id)sender
{
    ...
    if (isStart == NO) {
        ...
        NSString *TTSString = [NSString stringWithFormat:
                               @"运动开始，当前为 第%i阶段，持续时间 %@分钟，速度 %@千米每小时，坡度 %@度",
                               currentStep + 1,
                               [stepDic objectForKey:@"time"],
                               [stepDic objectForKey:@"speed"],
                               [stepDic objectForKey:@"slope"]];
    ...
    _currentSlopeLabel.text = [NSString stringWithFormat:@"%@°", [stepDic objectForKey:@"slope"]];
...
- (void)sportStart
{
    ...
    NSString *messageString = [NSString stringWithFormat:@"我在跑步机上开始了运动，当前为第%i阶段", currentStep+1];
...
- (void)sportOver
{
    ...
    [self commitSport];
    [self sendControlCommand:@"PBJ_STOP"];      //发送让跑步机停止的命令
    [self showText:@"运动已结束"];
...
-(void)processRequest:(NSString *)action info:(NSDictionary *)info error:(NSError *)error
{
    ...
    } else if ([action isEqualToString:Equip_Command_Request]) {
        //--------------跑步机控制命令发送--成功------------
        if (info) {

...
@interface WlhyZBarViewController () <ZBarReaderViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate>
...
@implementation WlhyZBarViewController
...
- (void)viewDidLoad
{
    ...
    if ([_purposeTag intValue] == 1) {
        [_scanBackgroundImageView setImage:[UIImage imageNamed:@"capture_equip.png"]];
    }
          ...
          fromImage: (UIImage*) image
{
    ...
    if (zbar_symbol_get_type(symbol) == ZBAR_QRCODE) {  // 是否QR二维码
        ...
        [_delegate finishGetBarCode:symbolStr];
    }
...
- (IBAction)inputOK:(id)sender
{
    ...
    if (_inputField.text.length <= 0) {
        [self showText:@"器械码不能为空"];
        return;
    }
    [_delegate finishGetBarCode:_inputField.text];
    [self dismissModalViewControllerAnimated:YES];
}
